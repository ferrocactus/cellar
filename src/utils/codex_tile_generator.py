...
def generate_tile(path_to_tiff, path_to_df, adata=None, palette=None, savepath=None):
    """
    ...
    for y in range(grid_y_len+1): # columns first
        ...
        for x in range(grid_x_len+1):
            ...
            if has_labels:
                for cell_index in my_cells.index:  # Iterate over cells - slow ...
                    cell_id = my_cells.loc[cell_index]['id']
                    cell_rid = my_cells.loc[cell_index]['rid']
                    label = adata.obs['labels'][cell_rid]
                    # Add 1 not to mess with black 0
                    tile[tile == cell_id] = label + 1
            else:
