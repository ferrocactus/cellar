...
cpdef DOUBLE _assign_labels_array(np.ndarray[floating, ndim=2] X,
                                  ...
                                  np.ndarray[INT, ndim=1] fixed_labels):
    ...
    for sample_idx in range(n_samples):
        min_dist = -1
        ####################################################
        # Update only if mask is set
        if mask is not None and mask[sample_idx] == 1:
            for center_idx in range(n_clusters):
                dist = 0.0
                ...
                ###############################################################
                # only update if mask is 1
                if min_dist == -1 or dist < min_dist:
                    min_dist = dist
                    labels[sample_idx] = center_idx
        else:
            min_dist = 0
            labels[sample_idx] = fixed_labels[sample_idx]
