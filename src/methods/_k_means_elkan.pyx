...
cdef update_labels_distances_inplace(
    ...
    for sample in range(n_samples):
        if mask[sample] == 1:
            ...
            labels[sample] = fixed_labels[sample]
...
def k_means_elkan(np.ndarray[floating, ndim=2, mode='c'] X_,
                  ...
                  np.ndarray[int, ndim=1, mode='c'] mask,
                  np.ndarray[int, ndim=1, mode='c'] fixed_labels,
    ...
    cdef int[:] mask_ = mask
    cdef int[:] fixed_labels_ = fixed_labels
    ...
    for iteration in range(max_iter):
        ...
        for point_index in range(n_samples):
            if mask[point_index] == 1:
                ...
                labels[point_index] = fixed_labels[point_index]
                upper_bounds[point_index] = 0
