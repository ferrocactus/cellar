...
class Pipeline:
    ...
    def create_objects(self, methods=None):
        dim_def = wrap("dim_reduction", self.config['methods']['dim_reduction'])
        clu_def = wrap("cluster", self.config['methods']['cluster'])
        eval_def = wrap("cluster_eval", self.config['methods']['cluster_eval'])
        vis_def = wrap("dim_reduction", self.config['methods']['visualization'])

        self.dim_obj = dim_def(self.verbose, **self.config[self.config['methods']['dim_reduction']])
        self.clu_obj = clu_def(self.verbose, **self.config[self.config['methods']['cluster']])
        self.eval_obj = eval_def(**self.config[self.config['methods']['cluster_eval']])
        self.vis_obj = vis_def(self.verbose, **self.config[self.config['methods']['visualization']])
